...
class GoalService {
  static async listGoals(filters, order = 'desc') {
    ...
    let years = [];
    if (goalsWithTransactionTotals.length > 0) {
      const userId = goalsWithTransactionTotals[0].userId;
      years = await GoalRepository.getYearsWithGoalsByUser(userId);
    }

    const metaData = { total, years };
    return {
      goals: goalsWithTransactionTotals,
      metaData
    };
  }
